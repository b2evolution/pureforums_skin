...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		sass: {
			development: {
				...
				files: {
					...
					'pureforums_header.css': 'pureforums_header.scss',
					'pureforums_main.css': 'pureforums_main.scss',
					'pureforums_footer.css': 'pureforums_footer.scss',
				}
			}
		},
		...
		concat: {
			skin_pureforums: {
				...
				src: ['pureforums_header.css', 'pureforums_main.css', 'pureforums_footer.css'],
				dest: 'pureforums.bundle.css',
			},
		},
		...
		cssmin: {
			style: {
				nonull: true, // Display missing files
				files: {
					'pureforums.bmin.css': [ 'pureforums.bundle.css' ],
				}
			},
		},
		...
		watch: {
			sass: {
				// Which files to watch (all .sass files)
				files: ['*.scss'],
				tasks: ['sass'],
				options: {
					nospawn: true,
	...
	// Default task(s):
	grunt.registerTask( 'default', [ 'sass', 'autoprefixer', 'concat', 'cssmin' ] );
};